...
if __name__ == "__main__":
    ...
    parser.add_argument("--dir", type=str, help="Enter your FCN_demo path, excluding / at the end")
    args = parser.parse_args()


cwd = args.dir  # current working directory
if cwd.endswith("/"):
    cwd = cwd[:-1]

# Add a path to a custom fork of TF-Slim
# Get it from here:
sys.path.append("{}/models/slim/".format(cwd)) # https://github.com/warmspringwinds/models/tree/fully_conv_vgg
sys.path.append(cwd) # Add path to the cloned library

checkpoints_dir = '{}/checkpoints/'.format(cwd)
log_folder = '{}/tf-image-segmentation/log_folder_fcn8/'.format(cwd)
fcn_16s_checkpoint_path = '{}/fcn_16s_checkpoint/model_fcn16s_final.ckpt'.format(cwd)
saving_dir= "{}/tf_image_segmentation/saver/model_fcn8s_final.ckpt".format(cwd)
...
with tf.Session()  as sess:
    ...
    for i in range(11127 * 10):
        ...
        if i % 11127 == 0:
            save_path = saver.save(sess, saving_dir)
